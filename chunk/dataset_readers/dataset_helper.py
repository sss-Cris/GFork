...
def convert_tags_to_wordpiece_IOtags(tags: List[str], offsets: List[int]) -> List[str]:
    new_tags = []
    j = 0
    for i, offset in enumerate(offsets):
        tag = tags[i]
...
def convert_dep_adj_to_wordpiece_dep_adj(
    ...
    for node_i, node_j in edges:
        # 如果任一端的 token 被截断，则跳过
        if node_i not in new_mapping or node_j not in new_mapping:
            continue

        nodes_i = new_mapping[node_i]
        nodes_j = new_mapping[node_j]

        # 将依存边映射到 wordpiece 级别
        edges_new = [(i_new, j_new) for i_new in nodes_i for j_new in nodes_j]
        new_dep_adj.extend(edges_new)

        # 添加 subword 内部的连接边（可选）
        if len(nodes_i) > 1:
            new_dep_adj.extend(get_edges_among_subwords(nodes_i))
        if len(nodes_j) > 1:
            new_dep_adj.extend(get_edges_among_subwords(nodes_j))

    return list(set(new_dep_adj))  # 去重后返回
...
def convert_tag_indices_to_wordpiece_indices(verb_indices: List[int], offsets: List[int]):
    j = 0
    new_verb_indices = []
    for i, offset in enumerate(offsets):
        indicator = verb_indices[i]
        while j < offset:
            ...
            j += 1
    # Add 0 indicators for cls and sep tokens.
    return ['Nil'] + new_verb_indices + ['Nil']

def convert_bound_indices_to_wordpiece_indices(tag_indices: List[int], offsets: List[int]):
    j = 0
    new_tag_indices = []
    new_tag_indices1 = []
    for i, offset in enumerate(offsets):
        indicator = tag_indices[i]
        while j < offset:
            new_tag_indices1.append(indicator)
            if indicator == 1:
                if j == offset - 1:
                     new_tag_indices.append(indicator)
                else:
                    new_tag_indices.append(0)
            else:
                new_tag_indices.append(0)
            j += 1
    # Add 1 indicators for cls and sep tokens.
    return [1] + new_tag_indices + [1]
